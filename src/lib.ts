...
export const CONFIG_DEFAULT = {
  name: createConfigDefaultItem({
    ...
    value: '',
    detail: 'Project name',
    documentation: 'Must be unique else will rewrite the project with the same name',
  }),
  ...
  for (let i = 0; serviceKeys[i]; i++) {
    ...
    if (isCustomService(image)) {
      ...
      (ports || []).forEach((portItem) => {
        ...
        if (!port) {
          res.push({
            msg: `Required parameter "${port}" in service "${item}" is missing`,
            data: '',
            exit: true,
            position: getPosition({
              config,
        ...
        if (proxy_path) {
          res = res.concat(
            checkLocation({ location: proxy_path, service: item, config, configText }, 'Proxy path')
          );
          if (_type === 'php') {
            res.push({
      ...
      // Check enviroment is array
      if (environment) {
        if (!Array.isArray(environment)) {
          res.push({
            msg: 'Environment must be an array',
            data: `Received "${typeof environment}"`,
            exit: true,
            position: getPosition({
              config,
              configText,
